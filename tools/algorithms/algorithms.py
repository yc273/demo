...
class AlgorithmsApi:
    def __init__(self, brand: str):        
        ...
        # 常量
        # self.offset = [+0.01, +0.02] #焊枪
        # self.offset1 = [-0.03, -0.03, -0.1, 0, 0, 0]
        # self.offset2 = [self.offset[0], self.offset[1], 0, 0, 0, 0]
        # #深度由200mm调整至270mm
        # self.offset3 = [0,0,-0.2,0,0,0]
        # self.offset4 = [0,0,0.028,0,0,0]

        self.offset = [+0.01, -0.02]#拧螺套
        self.offset1 = [-0.03, -0.05, -0.18, 0, 0, 0]
        self.offset2 = [self.offset[0], self.offset[1], 0, 0, 0, 0]
        #深度由200mm调整至270mm
 ...
 #region 角度
    ...
    def calculate_angle_adjustment(self, measured_z: List[float]) -> List[float]:
        ...
        rx_tcp, ry_tcp, rz_tcp = RotationCalculator.tcp_rotation_calculation(
            self.calibration_z_axis,measured_z, self.brand )
    ...
    def is_angle_adjusted_perfectly(self, measured_z: List[float]) -> bool:
        ...
        try: 
            ...
            result = abs(angle_offset[3]) < 0.003 and abs(angle_offset[4]) < 0.003#TODO
            if result: 
                self.log("当前角度已经完全调整！")
                return True
            else:
