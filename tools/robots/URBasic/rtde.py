...
class RTDE(threading.Thread): #, metaclass=Singleton
    ...
    Example:
    rob = RTDE('192.168.56.101', 'rtde_configuration.xml')
    rob.close_rtde()
    '''
    ...
    def __init__(self, robotModel, conf_filename=None):
        ...
        if conf_filename is None:
            conf_filename = __file__[0:__file__.find('URBasic')] + 'URBasic/rtdeConfiguration.xml'
            print(conf_filename)
            if not os.path.isfile(conf_filename):
                conf_filename = __file__[0:__file__.find('URBasic')] + 'URBasic/rtdeConfigurationDefault.xml'
                print(conf_filename)
    ...
    def __updateModel(self, rtde_data_package):
        ...
        if(self.__robotModel.dataDir['timestamp'] != None):
            ...
            if(delta > 0.00800001):
                ...
                # print("Lost some RTDE at " + str(rtde_data_package['timestamp']) + " - " + str(
                #     delta * 1000) + " milliseconds since last package")
                pass
    ...
    def __verifyControllerVersion(self, data):
        ...
        if major and minor and bugfix:
            ...
            print(
                'Controller version: ' + str(major) + '.' + str(minor) + '.' + str(bugfix) + '-' + str(build))
            if major <= 3 and minor <= 2 and bugfix < 19171:
                # self._logger.error("Please upgrade your controller to minimum version 3.2.19171")
                print("Please upgrade your controller to minimum version 3.2.19171")
    ...
    def __decodePayload(self, cmd, payload):
        ...
        if cmd == Command.RTDE_REQUEST_PROTOCOL_VERSION:
            ...
            #print('Unknown RTDE command type: ' + chr(cmd))
    ...
    def run(self):
        ...
        while (time.time()-t0<self.__reconnectTimeout) and self.__conn_state != ConnectionState.STARTED:
            ...
            #time.sleep(0.5)
        if self.__conn_state != ConnectionState.STARTED:
            print("RTDE connection failed - unable to establish initial connection")
            return
        ...
        while (not self.__stop_event) and (time.time()-t0<self.__reconnectTimeout):
            try:
                self.__receive()
                t0 = time.time()
                consecutive_failures = 0  # 成功接收，重置失败计数
                last_failure_printed = False
            except Exception:
                if self.__conn_state >= ConnectionState.STARTED:
                ...
                # 只在连续失败达到阈值或首次失败时打印错误
                if consecutive_failures >= max_consecutive_failures and not last_failure_printed:
                    print(f"RTDE reconnection failed after {consecutive_failures} consecutive attempts")
                    last_failure_printed = True
                ...
                if self.__conn_state == ConnectionState.STARTED:
                    if consecutive_failures > 0:  # 只有在确实有过失败后才打印恢复信息
                        print("RTDE connection restored")
                    consecutive_failures = 0
                    last_failure_printed = False
