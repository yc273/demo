...
from . import robotConnector, manipulation
import numpy as np
import time
...
class UrScript(object):
    ...
    Example:
    rob = UrScript('192.168.56.101', rtde_conf_filename='rtde_configuration.xml')
    self.close_rtc()
    '''
    ...
    def __init__(self, host, robotModel, hasForceTorque=False):
        ...
        '''
        self.robotConnector = robotConnector.RobotConnector(robotModel, host, hasForceTorque)
        # time.sleep(200)
        ...
        #     time.sleep(1)
        # 添加超时机制，防止在无法连接时无限期阻塞
        timeout = 3  # 超时
        start_time = time.time()
        while (self.robotConnector.RobotModel.ActualTCPPose() is None):
            # 检查是否超时
            if time.time() - start_time > timeout:
                raise TimeoutError(f"连接UR机器人 {host} 超时")
            time.sleep(1)
        print(self.robotConnector.RobotModel.ActualTCPPose())
...
'''
    ...
    def pose_to_transformation_matrix(pose):
        '''
        Convert UR pose format [x, y, z, rx, ry, rz] to 4x4 transformation matrix
        '''
        import math
        from numpy import array, sin, cos, pi, sqrt, trace, acos
        ...
        if angle > 1e-6:
            ...
            R = array([
                ...
                [ky*kx*vt + kz*st, ky*ky*vt + ct,    ky*kz*vt - kx*st],
                [kz*kx*vt - ky*st, kz*ky*vt + kx*st, kz*kz*vt + ct]
            ])
        else:
            R = array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        
        return manipulation.RpToTrans(R, [x, y, z])
    
    @staticmethod
    def transformation_matrix_to_pose(T):
        '''
        Convert 4x4 transformation matrix to UR pose format [x, y, z, rx, ry, rz]
        '''
        from numpy import array, sqrt, atan2, trace
        from numpy import array, sqrt, atan2, trace, acos, sin

        R, p = manipulation.TransToRp(T)
        ...
        if angle > 1e-6:
            ...
            rx, ry, rz = kx * angle, ky * angle, kz * angle
        else:
            rx, ry, rz = 0, 0, 0
        
        return array([p[0], p[1], p[2], rx, ry, rz])
    ...
    def pose_add(p_1, p_2):
        ...
        '''
        Trans_1 = UrScript.pose_to_transformation_matrix(p_1)
        Trans_2 = UrScript.pose_to_transformation_matrix(p_2)
        Trans_3 = np.matmul(Trans_1, Trans_2)
        p_3 = UrScript.transformation_matrix_to_pose(Trans_3)
        return p_3
